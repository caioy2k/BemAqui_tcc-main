const API_URL = "https://bemaqui-tcc-main.onrender.com/trades";
  ...
  trades.forEach((trade) => {
    ...
    row.innerHTML = `
      <td>
        ...
        <div class="cell-muted">ID: ${escapeHtml(tradeId)}</div>
      </td>
...
function showModal() {
  ...
  if (window.bootstrap && bootstrap.Modal) {
    const modalInstance = bootstrap.Modal.getOrCreateInstance(tradeModal);
    modalInstance.show();
    return;
  }
...
function closeTradeModal() {
  ...
  if (window.bootstrap && bootstrap.Modal) {
    const modalInstance = bootstrap.Modal.getInstance(tradeModal);
    if (modalInstance) {
      modalInstance.hide();
    }
  } else {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (tradeModal && !window.bootstrap) {
